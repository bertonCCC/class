/* This version is has a more generalized construction of the segment tree
 * If you can't find the identity , an element I such that for all A,
 * A + I = I + A = A (where the + sign is combining segments), then you
 * can use this verison, where we identify and ignore segments returned
 * in the no-overlap case
 */
import java.io.*;

public class dmpg17g2_2{
  ...
  static Segment query(int qleft, int qright, int left, int right, int index){
    if(left > qright || right < qleft) return new Segment(-INF, 0, 0, 0);
    else if(left >= qleft && right <= qright) return seg[index];
    else{
      int mid = (left + right) / 2;
      Segment leftQuery = query(qleft, qright, left, mid, index * 2);
      Segment rightQuery = query(qleft, qright, mid + 1, right, index * 2 + 1);
      if(leftQuery.maxSum == -INF) return rightQuery;
      if(rightQuery.maxSum == -INF) return leftQuery;
